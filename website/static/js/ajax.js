...
var table_dataSlangword = $('#table_dataSlangword').DataTable({
    ...
    "columns": [
        ...
        { data: 'kata_asli' },
        {
            data: null,
            "defaultContent": `
                <button type="button" value="update" class="btn btn-primary mb-1"><i class='bx bx-edit'></i> Edit</button>
                <button type="button" value="delete" class="btn btn-danger mb-1"><i class='bx bx-trash'></i> Delete</button>								
            `
        },
    ],
});
...
$('#table_dataPreprocessing tbody').on( 'click', 'button', function () {
    ...
    if($(this).prop("value") == 'modalTweetAsli') {
        ...
        $("#modalLihatTweetAsli").find("p[id='cleansing']").html(data['cleansing']);
...
$('#split_data').click(function() {
    ...
    if(jumlah_data_with_label > 0 && form_dataArray[0]['name'].trim() == 'rasio' && (form_dataArray[0]['value'] == '9:1' || form_dataArray[0]['value'] == '8:2' || form_dataArray[0]['value'] == 'custom')) {
        var content =	"";
...
$('#btn-prediksi').click(function() {
    ...
    var content =	"";
        ...
        $.ajax({
            ...
            success     : function(response) {
                content += `<p>Prediksi :` + response.asd + `</p>`;
                $('#content_prediksi').html(content);
                $(".lds-ring").remove();
            },
            error     : function(x) {
