...
class SplittingController:
    ...
    def count_dataWithLabelNet(self):
        instance_model = Models('SELECT COUNT(id) as jumlah FROM tbl_tweet_preprocessing WHERE label = "netral"')
        data_net = instance_model.select()
        return data_net[0]['jumlah']
    ...
    def count_data_test(self):
        instance_model = Models('SELECT COUNT(id) as jumlah FROM tbl_data_test')
        data_test = instance_model.select()
        return data_test[0]['jumlah']
    ...
    def add_dataSplit(self):
        ...
        if rasio == '9:1':
            ...
            rasio_train = int(request.form['custom_value']) / 10
        ...
        # Membagi data train dan test dengan Stratified Random Sampling (Rasio label pada data latih dan uji sesuai dengan rasio label sebelum split data)
        train = df.groupby('label', group_keys=False).apply(lambda x: x.sample(frac=rasio_train))
        ...
        for index, data in train.iterrows():
            train_tuples = (data['id'], data['text'], data['clean_text'], data['user'], str(datetime.strftime(datetime.strptime(str(data['created_at']),'%Y-%m-%d %H:%M:%S'), '%Y-%m-%d %H:%M:%S')), data['label'])
            data_simpan_train.append(train_tuples)

        instance_model = Models('INSERT IGNORE tbl_data_train(id, text, clean_text, user, created_at, label) VALUES (%s, %s, %s, %s, %s, %s)')
        instance_model.query_sql_multiple(data_simpan_train)

        for index, data in test.iterrows():
            test_tuples = (data['id'], data['text'], data['clean_text'], data['user'], str(datetime.strftime(datetime.strptime(str(data['created_at']),'%Y-%m-%d %H:%M:%S'), '%Y-%m-%d %H:%M:%S')), data['label'])
            data_simpan_test.append(test_tuples)

        instance_model = Models('INSERT IGNORE tbl_data_test(id, text, clean_text, user, created_at, label) VALUES (%s, %s, %s, %s, %s, %s)')
